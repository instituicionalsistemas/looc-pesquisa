// components/ResearcherModal.tsx
...
const emptyResearcher: Omit<Researcher, 'id'> = {
    name: '',
    email: '',
    phone: '',
...
const ResearcherModal: React.FC<ResearcherModalProps> = ({ isOpen, onClose, onSave, researcher }) => {
    const [formData, setFormData] = useState<Partial<Researcher>>(researcher || emptyResearcher);

    useEffect(() => {
        setFormData(researcher || emptyResearcher);
    }, [researcher, isOpen]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const researcherToSave: Researcher = {
            id: researcher?.id || `res_${Date.now()}`,
            isActive: researcher?.isActive ?? true,
            ...emptyResearcher,
            ...formData,
        };
        onSave(researcherToSave);
    };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="gender" name="gender" value={formData.gender || ''} onChange={handleChange} className="w-full input-style">
